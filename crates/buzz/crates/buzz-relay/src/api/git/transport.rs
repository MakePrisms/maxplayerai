...
//!
//! Auth: NIP-98 on all routes (clone + push). Relay membership is additionally
//! required on every route by default; the READ routes (`info/refs` +
//! `git-upload-pack`) drop ONLY the membership requirement — NIP-98 still
//! enforced — when `config.git_public_read` is enabled. Push always requires
//! membership.
...
impl axum::extract::FromRequestParts<Arc<AppState>> for GitAuth {
    ...
    async fn from_request_parts(
        ...
        // Non-read git routes (push + anything else) always require membership.
        authenticate_git(parts, state, true).await
    }
}

/// NIP-98 auth extractor for git READ routes (`info/refs`, `git-upload-pack` —
/// clone/fetch). Identical to [`GitAuth`], including full NIP-98 signature
/// verification, EXCEPT that when `config.git_public_read` is enabled the
/// relay-membership (NIP-43) requirement is lifted for these READ routes only.
/// Push (`git-receive-pack`) always uses [`GitAuth`] and stays member-gated.
/// Flag off ⇒ `require_membership == true` ⇒ behavior identical to [`GitAuth`].
pub struct GitReadAuth(pub GitAuth);

impl axum::extract::FromRequestParts<Arc<AppState>> for GitReadAuth {
    type Rejection = Response;
    ...
    async fn from_request_parts(
        ...
        let require_membership = !state.config.git_public_read;
        authenticate_git(parts, state, require_membership)
            .await
            .map(GitReadAuth)
    }
}
...
async fn authenticate_git(
    ...
    // the NIP-98 signature check above always runs. Push and every other
    // caller pass `require_membership = true`, leaving this gate unchanged.
    //
    // Git cannot carry a standalone x-auth-tag header through the credential-
    // helper protocol, so agents attach their NIP-OA attestation to the signed
    // NIP-98 event (upstream #2120), matching the WebSocket NIP-42 flow.
...
pub async fn upload_pack(
    ...
    // the subprocess lifetime and is never released while git runs. This is
    // stricter than the streaming permit-through-drain (#1916 SEC-018), which
    // had to keep the permit alive across a slow client's body drain.
    let _permit = acquire_git_permit(&state, "upload_pack")?;
    ...
    // Fork hardening (PR#1, re-carried onto #2167): BUFFERED read + FAIL CLOSED.
    //
    // Upstream's streaming `upload_pack` (`stream_git_read`) commits the HTTP 200
    // head before git runs, so an upload-pack failure — e.g. a hydrated repo that
    // cannot serve a v0 multi-round negotiation — reaches the client as a silent
    // empty/truncated 200 ("remote end hung up") with git's stderr discarded.
    // That mask is exactly why the money-path clone hang stayed invisible until a
    // paying buyer hit it. Upstream #2167 ("bound and observe") added a stream
    // timeout + byte metrics but never checks the child's exit status, so it does
    // NOT subsume this fix — see plans/buzz-sync-recarry-staging-20260721.md.
    //
    // Buffering waits for the subprocess, keeps #2167's wall-clock
