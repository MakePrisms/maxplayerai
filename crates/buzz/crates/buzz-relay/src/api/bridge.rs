...
/// Top-level filter keys permitted on the anonymous public-repo READ bypass.
///
/// Only these plain Nostr filter fields are allowed on the anon `/query` path.
/// Every one is re-applied by the catchall serve path (`filters_match`) against
/// the kind-constrained results, and none of them selects an alternate,
/// channel-spanning dispatch (feed / thread / channel-window / search).
///
/// This is deliberately an ALLOWLIST, not a denylist of extension keys. The
/// serve path reads a growing set of extension/dispatch keys straight from the
/// raw JSON — `feed_types`, `depth_limit`, `top_level`, `search`, `before_id`,
/// `page`, `thread_cursor`/`threadCursor`, `thread_cursor_id`, `search_mode`,
/// `search_page`, `include_aux`, `include_summaries` — that `nostr::Filter`
/// silently drops. Permitting a fixed safe set (and rejecting everything else)
/// fails closed against every one of those keys *and* any dispatch key added in
/// future, instead of relying on a denylist that must be kept in sync by hand.
fn is_public_repo_read_filter_key(key: &str) -> bool {
    // Standard scalar/array filter fields handled by the catchall + filters_match.
    matches!(key, "kinds" | "authors" | "ids" | "since" | "until" | "limit")
        // Standard single-letter tag filters (`#a`, `#d`, `#e`, `#p`, …). The
        // catchall re-applies these via `filters_match`; a channel (`#h`) scope
        // matches no channel-less repo event and yields nothing for a non-member.
        || (key.len() == 2 && key.starts_with('#') && key.as_bytes()[1].is_ascii_alphabetic())
}

/// Whether a `/query` request body is a plain public-repo READ — the ONLY shape
/// the anonymous (non-member) bypass in [`query_events`] will serve.
///
/// Operates on the RAW JSON, NOT the typed `nostr::Filter`: `nostr::Filter`
/// silently drops the CLI extension fields the serve path dispatches on, so a
/// typed check would wave through `{"kinds":[30617],"feed_types":["activity"]}`
/// while the serve path fans out to channel chat. Requires a non-empty JSON
/// array where EVERY element is an object that (1) carries no key outside
/// [`is_public_repo_read_filter_key`] and (2) restricts `kinds` to a non-empty
/// subset of {30617, 30618}. Any parse failure, non-object element, missing or
/// other kind, or any extension/dispatch key → `false` (fail closed to the
/// membership gate). Both kinds are `is_global_only_kind` (never channel-scoped)
/// and the catchall re-applies `filters_match`, so no channel data can leak.
fn body_is_public_repo_read_only(body: &[u8]) -> bool {
    let Ok(Value::Array(filters)) = serde_json::from_slice::<Value>(body) else {
        return false;
    };
    !filters.is_empty() && filters.iter().all(json_filter_is_public_repo_read_only)
}

/// A single raw-JSON filter object is a plain public-repo read: an object whose
/// every key is allowlisted and whose `kinds` is a non-empty subset of
/// {30617, 30618}.
fn json_filter_is_public_repo_read_only(filter: &Value) -> bool {
    let Some(obj) = filter.as_object() else {
        return false;
    };
    // Any non-allowlisted key (every serve-path extension/dispatch key) → reject.
    if !obj.keys().all(|k| is_public_repo_read_filter_key(k)) {
        return false;
    }
    // `kinds` must be present, a non-empty array, and ⊆ {30617, 30618}.
    match obj.get("kinds") {
        Some(Value::Array(kinds)) => {
            !kinds.is_empty()
                && kinds.iter().all(|k| {
                    k.as_u64().is_some_and(|kind| {
                        kind == buzz_core::kind::KIND_GIT_REPO_ANNOUNCEMENT as u64
                            || kind == buzz_core::kind::KIND_GIT_REPO_STATE as u64
                    })
                })
        }
        _ => false,
...
/// Whether the anonymous public-repo READ carve should pin a catchall query to
/// community-global (channel-less) rows by setting [`buzz_db::EventQuery::global_only`].
///
/// Returns `true` only when ALL hold:
/// 1. `skip` — the request took the flag-gated anon repo-read bypass
///    (`git_public_read` on AND [`body_is_public_repo_read_only`]); `false`
///    otherwise, so the carve is fully inert unless the flag is on.
/// 2. The query is not already channel-scoped (`global_only` and `channel_id`
///    are mutually exclusive in [`buzz_db::event::query_events`]; a member's
///    `#h`-scoped repo filter keeps its own scope).
/// 3. Every requested kind is a public repo kind {30617, 30618}.
///
/// When set, the emitted WHERE becomes exactly `community_id = $tenant AND kind
/// IN (30617,30618) AND channel_id IS NULL AND deleted_at IS NULL`: channel-scoped
/// repo events (`channel_id IS NOT NULL`) are never fetched, so repo content
/// published inside a channel stays gated. Independently triple-scoped —
/// kind ∈ {30617,30618} ∧ channel_id IS NULL ∧ community_id = $tenant.
fn pin_repo_read_to_global_only(skip: bool, query: &buzz_db::EventQuery) -> bool {
    skip && query.channel_id.is_none()
        && query.kinds.as_deref().is_some_and(|ks| {
            !ks.is_empty()
                && ks.iter().all(|&k| {
                    k == buzz_core::kind::KIND_GIT_REPO_ANNOUNCEMENT as i32
                        || k == buzz_core::kind::KIND_GIT_REPO_STATE as i32
                })
        })
}
...
async fn query_events_authed(
    ...
    // Anonymous public-repo READ bypass (flag-gated, OFF by default). When
    // `git_public_read` is on AND every filter restricts `kinds` to the public
    // repo allowlist {30617, 30618}, skip the relay-membership gate and serve
    // the query. `verify_bridge_auth` above already validated a NIP-98 signature
    // (member or not), so this lifts ONLY membership — not authentication — and
    // only for two `is_global_only_kind` kinds that carry no channel-scoped
    // data. Any other kind, or a filter with no `kinds` restriction, falls
    // through to the gate unless the deployment-wide open-read flag below is on.
    let skip_membership_for_public_repo_read =
        state.config.git_public_read && body_is_public_repo_read_only(body);

    // Open relay READ bypass (flag-gated, OFF by default). `BUZZ_OPEN_READ`
    // lifts the relay-membership gate for bridge queries across all kinds on
    // the open unit. Authentication, p-gated/engram/author-only result gates,
    // and channel visibility checks still run below; this flag only changes the
    // relay-member requirement.
    let skip_membership_for_open_read = state.config.open_read;
    if !(skip_membership_for_public_repo_read || skip_membership_for_open_read) {
    ...
    for (idx, (raw, filter)) in raw_filters.iter().zip(filters.iter()).enumerate() {
        ...
        // SQL-layer anonymous public-repo READ carve (flag-gated, OFF by
        // default). When the anon repo-read bypass is active for this request,
        // pin the repo-list fetch to community-global (channel-less) rows so the
        // emitted WHERE is exactly:
        //   community_id = $tenant AND kind IN (30617,30618)
        //     AND channel_id IS NULL AND deleted_at IS NULL
        // `global_only` is the existing EventQuery flag that adds
        // `AND channel_id IS NULL`; setting it here means channel-scoped events
        // (`channel_id IS NOT NULL`) are never fetched, so repo content published
        // inside a channel stays gated. Independently triple-scoped in the SQL —
        // kind ∈ {30617,30618} ∧ channel_id IS NULL ∧ community_id = $tenant —
        // and fully inert unless `git_public_read` + a repo-read-only body. Guard
        // on `channel_id.is_none()` because `global_only` and `channel_id` are
        // mutually exclusive (a member's `#h`-scoped repo filter keeps its scope).
        if pin_repo_read_to_global_only(skip_membership_for_public_repo_read, &query) {
        ...
        match filter_events {
            Ok(stored_events) => {
                for se in stored_events {
                    // Channel-visibility gate. The anonymous public-repo READ
                    // carve is enforced at the SQL layer above (`global_only`
                    // pins those kinds to `channel_id IS NULL`), so channel-less
                    // repo rows admit here unconditionally and channel-scoped
                    // rows are never fetched — no per-event kind exception is
                    // needed. This supersedes the post-fetch carve from
                    // d9cef197, which was the wrong layer (per sprout) and also
                    // over-broad: it bypassed this check for ALL 30617/30618
                    // events, including channel-scoped ones.
                    if !event_in_accessible_channel(&se, &accessible_channels) {
                        continue;
                    }
...
mod tests {
    ...
    // ---- anonymous public-repo READ bypass guard (BUZZ_GIT_PUBLIC_READ) ----
    // These lock the anon `/query` path to the plain catchall repo-list. The
    // guard operates on RAW JSON because `nostr::Filter` drops the serve-path
    // extension keys; a typed check would wave the leak vectors through.

    #[test]
    fn anon_repo_read_accepts_plain_repo_list() {
        assert!(body_is_public_repo_read_only(br#"[{"kinds":[30617]}]"#));
        assert!(body_is_public_repo_read_only(br#"[{"kinds":[30618]}]"#));
        assert!(body_is_public_repo_read_only(
            br#"[{"kinds":[30617,30618]}]"#
        ));
        // Standard plain-filter fields are allowed (re-applied by filters_match).
        assert!(body_is_public_repo_read_only(
            br#"[{"kinds":[30617],"authors":["ab"],"limit":50,"since":1,"until":9}]"#
        ));
        assert!(body_is_public_repo_read_only(
            br##"[{"kinds":[30618],"#d":["repo"],"#a":["30617:ab:repo"]}]"##
        ));
        assert!(body_is_public_repo_read_only(
            br#"[{"kinds":[30617],"ids":["ff"]}]"#
        ));
        // Multiple filters, each a valid repo-list filter.
        assert!(body_is_public_repo_read_only(
            br#"[{"kinds":[30617]},{"kinds":[30618]}]"#
        ));
    }

    #[test]
    fn anon_repo_read_rejects_extension_dispatch_vectors() {
        // The refuted leak + same-class vectors: each carries a serve-path
        // dispatch/extension key that `nostr::Filter` silently drops. Every one
        // MUST fall through to the membership gate (guard returns false).
        for body in [
            br#"[{"kinds":[30617],"feed_types":["activity"]}]"#.as_slice(),
            br#"[{"kinds":[30617],"feed_types":["mentions"]}]"#.as_slice(),
            br#"[{"kinds":[30617],"feed_types":["needs_action"]}]"#.as_slice(),
            br#"[{"kinds":[30617],"depth_limit":3}]"#.as_slice(),
            br#"[{"kinds":[30617],"top_level":true}]"#.as_slice(),
            br#"[{"kinds":[30617],"search":"x"}]"#.as_slice(),
            br#"[{"kinds":[30617],"before_id":"aa"}]"#.as_slice(),
            br#"[{"kinds":[30617],"page":2}]"#.as_slice(),
            br#"[{"kinds":[30617],"search_page":2}]"#.as_slice(),
            br#"[{"kinds":[30617],"search_mode":"prefix"}]"#.as_slice(),
            br#"[{"kinds":[30617],"thread_cursor":123}]"#.as_slice(),
            br#"[{"kinds":[30617],"thread_cursor_id":"aa"}]"#.as_slice(),
            br#"[{"kinds":[30617],"include_aux":true}]"#.as_slice(),
            br#"[{"kinds":[30617],"include_summaries":true}]"#.as_slice(),
            // Any unknown key at all → fail closed (future dispatch keys too).
            br#"[{"kinds":[30617],"totally_new_dispatch_key":1}]"#.as_slice(),
            // An extension key on a SECOND filter still poisons the whole request.
            br#"[{"kinds":[30617]},{"kinds":[30617],"feed_types":["activity"]}]"#.as_slice(),
        ] {
            assert!(
                !body_is_public_repo_read_only(body),
                "extension/dispatch vector must fall through to the membership gate: {}",
    ...
    #[test]
    fn anon_repo_read_rejects_non_allowlisted_kinds_and_degenerate_bodies() {
        for body in [
            br#"[{"kinds":[9]}]"#.as_slice(),                   // chat
            br#"[{"kinds":[45001]}]"#.as_slice(),               // forum
            br#"[{"kinds":[45003]}]"#.as_slice(),               // job
            br#"[{"kinds":[30617,9]}]"#.as_slice(),             // mixed repo + chat
            br#"[{"kinds":[]}]"#.as_slice(),                    // empty kinds
            br#"[{"authors":["ab"]}]"#.as_slice(),              // no kinds (unrestricted)
            br#"[{}]"#.as_slice(),                              // no kinds
            br#"[]"#.as_slice(),                                // empty array
            br#"[{"kinds":[30617]},{"kinds":[9]}]"#.as_slice(), // one bad filter poisons
            br#"{"kinds":[30617]}"#.as_slice(),                 // object, not array
            br#"[42]"#.as_slice(),                              // non-object element
            br#"[{"kinds":"30617"}]"#.as_slice(),               // kinds not an array
            br#"[{"kinds":[30617.5]}]"#.as_slice(),             // non-integer kind
            b"not json".as_slice(),                             // parse failure
        ] {
            assert!(
                !body_is_public_repo_read_only(body),
                "non-repo/degenerate body must fall through to the membership gate: {}",
                std::str::from_utf8(body).unwrap_or("<non-utf8>")
            );
        }
    }

    // ---- SQL-layer anon public-repo READ carve (`global_only` pin) ----
    // Locks the decision that sets `EventQuery.global_only` so the emitted WHERE
    // is exactly `community_id = $tenant AND kind IN (30617,30618) AND channel_id
    // IS NULL AND deleted_at IS NULL`. The carve is triple-scoped and inert
    // unless the anon repo-read bypass (`skip`) is active.
    ...
    fn repo_read_query(
        ...
        let mut q = buzz_db::EventQuery::for_community(community);
        q.kinds = kinds;
        q.channel_id = channel;
        q
    }

    #[test]
    fn pin_repo_read_global_only_true_for_repo_kinds_under_bypass() {
        // Bypass active + repo kinds + no channel scope → pin to channel-less.
        assert!(pin_repo_read_to_global_only(
            true,
            &repo_read_query(Some(vec![30617]), None)
        ));
        assert!(pin_repo_read_to_global_only(
            true,
            &repo_read_query(Some(vec![30618]), None)
        ));
        assert!(pin_repo_read_to_global_only(
            true,
            &repo_read_query(Some(vec![30617, 30618]), None)
        ));
    }

    #[test]
    fn pin_repo_read_global_only_false_unless_bypass_active() {
        // Flag off / non-repo-read body ⇒ `skip == false` ⇒ never pin, so the
        // carve is fully OFF and behavior is unchanged from before the flag.
        assert!(!pin_repo_read_to_global_only(
            false,
            &repo_read_query(Some(vec![30617]), None)
        ));
    ...
    #[test]
    fn pin_repo_read_global_only_false_for_non_repo_or_degenerate_kinds() {
        // Non-repo kind, mixed kinds, unrestricted (None), and empty kinds must
        // never pin — the carve only ever narrows the two repo kinds.
        assert!(!pin_repo_read_to_global_only(
            true,
            &repo_read_query(Some(vec![9]), None)
        ));
        assert!(!pin_repo_read_to_global_only(
            true,
            &repo_read_query(Some(vec![30617, 9]), None)
        ));
        assert!(!pin_repo_read_to_global_only(
            true,
            &repo_read_query(None, None)
        ));
        assert!(!pin_repo_read_to_global_only(
            true,
            &repo_read_query(Some(vec![]), None)
        ));
    }

    #[test]
    fn pin_repo_read_global_only_false_when_channel_scoped() {
        // A repo filter that carries an (accessible) `#h` channel scope keeps it:
        // `global_only` and `channel_id` are mutually exclusive in the DB layer.
        assert!(!pin_repo_read_to_global_only(
            true,
            &repo_read_query(Some(vec![30617]), Some(uuid::Uuid::from_u128(1)))
        ));
    }
