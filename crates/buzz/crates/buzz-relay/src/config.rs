...
pub struct Config {
    ...
    /// When `true`, anonymous (non-member) READ of public git repos is allowed:
    /// the `/query` bridge serves kind:30617 (repo announcement) + kind:30618
    /// (ref state) without the relay-membership gate, and the git smart-HTTP READ
    /// routes (`info/refs`, `git-upload-pack` = clone/fetch) skip the membership
    /// check. NIP-98 signature verification still runs on the git routes — only
    /// the membership requirement is lifted. Chat, DMs, all event writes, and git
    /// push (`git-receive-pack`) stay fully member-gated regardless of this flag.
    ///
    ...
    /// membership gate. This is intended only for explicitly open relay units:
    /// writes, channel-scoped rows, p-gated result rows, engrams, and author-only
    /// events remain protected by their existing gates.
    ///
    /// Default: `false` (zero behavior change). Set via `BUZZ_OPEN_READ=true`.
    pub open_read: bool,
...
impl Config {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        // Off by default. When on, anonymous non-members may READ public git
        // repos (kind:30617/30618 via /query, and the git clone/fetch routes);
        // writes and push stay member-gated. See the `git_public_read` field doc.
        let git_public_read = std::env::var("BUZZ_GIT_PUBLIC_READ")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);
...
mod tests {
    ...
    fn defaults_are_valid() {
        ...
        assert!(
            !config.git_public_read,
            "git_public_read should default to false (anon git read is opt-in)"
        );
        assert!(
            !config.open_read,
            "open_read should default to false (anon relay read is opt-in)"
        );
        assert!(
    ...
    #[test]
    fn open_read_can_be_enabled_for_open_relay_units() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("BUZZ_OPEN_READ", "true");
        let config = Config::from_env().expect("config");
        std::env::remove_var("BUZZ_OPEN_READ");
        assert!(
            config.open_read,
            "BUZZ_OPEN_READ=true must enable anonymous open relay reads"
        );
    }
