...
pub async fn handle_req(
    ...
    let (conn_id, pubkey_bytes, token_channel_ids, anonymous_open_read) = {
        ...
        match &*auth {
            ...
            _ if state.config.open_read => {
                ...
                (conn.conn_id, Vec::new(), None, true)
            }
            _ => {
    ...
    let mut accessible_channels = if anonymous_open_read {
        // Open-relay anon read (BUZZ_OPEN_READ): unauthenticated, so there is no
        // membership-scoped access set to resolve.
        Vec::new()
    ...
    if channel_id.is_none() {
        let authed_pubkey_hex = if anonymous_open_read {
            String::new()
        } else {
    ...
    let filter_queries: Vec<(usize, Option<uuid::Uuid>, EventQuery)> = filters
        ...
        .map(|(idx, filter)| {
            ...
            let params = if anonymous_open_read {
                open_read_filter_to_query_params(
                    filter,
                    per_filter_channel,
                    conn.tenant.community(),
                )
...
/// Open-relay anon read (BUZZ_OPEN_READ): build a public-read query. A global
/// (channel-less) subscription is served as `global_only` rather than scoped to
/// a membership access set (there is none for anon read).
fn open_read_filter_to_query_params(
    filter: &Filter,
    channel_id: Option<uuid::Uuid>,
    community: buzz_core::tenant::CommunityId,
) -> EventQuery {
...
mod tests {
    ...
    #[test]
    fn open_read_query_params_pin_unscoped_reads_to_global_rows() {
        let community = buzz_core::tenant::CommunityId::from_uuid(uuid::Uuid::nil());
        let filter = Filter::new().kind(nostr::Kind::Custom(5109));

        let query = open_read_filter_to_query_params(&filter, None, community);

        assert!(query.global_only);
        assert!(query.channel_id.is_none());
        assert_eq!(query.kinds, Some(vec![5109]));
    }

    #[test]
    fn open_read_query_params_preserve_explicit_channel_scope() {
        let community = buzz_core::tenant::CommunityId::from_uuid(uuid::Uuid::nil());
        let channel = uuid::Uuid::new_v4();
        let filter = Filter::new().kind(nostr::Kind::Custom(5109));

        let query = open_read_filter_to_query_params(&filter, Some(channel), community);
