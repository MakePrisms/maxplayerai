...
const KIND_MOBEE_JOB_RECEIPT: u32 = 3400;
const KIND_MOBEE_JOB_OFFER: u32 = 5109;
...
/// True when `BUZZ_OPEN_INGEST=true|1` is set at process start. Read once via
/// `OnceLock` — the flag never changes at runtime. When enabled, the catch-all
/// arm of `required_scope_for_kind` maps unknown event kinds to
/// `Scope::MessagesWrite` instead of rejecting them, turning the relay into an
/// open ingest surface (paired with `BUZZ_REQUIRE_RELAY_MEMBERSHIP=false` and
/// `BUZZ_REQUIRE_AUTH_TOKEN=false` for an anonymous open relay). Off by default
/// — every other production configuration keeps the strict allowlist.
fn open_ingest_enabled() -> bool {
    static V: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *V.get_or_init(|| {
        std::env::var("BUZZ_OPEN_INGEST")
            .map(|s| s == "true" || s == "1")
            .unwrap_or(false)
    })
}
...
///
/// Returns `Err` for unknown kinds — the relay rejects them. When
/// `BUZZ_OPEN_INGEST=true` is set, the catch-all instead maps unknown kinds to
/// `Scope::MessagesWrite`, allowing an anonymous open relay to accept them.
fn required_scope_for_kind(kind: u32, event: &Event) -> Result<Scope, &'static str> {
    match kind {
        ...
        // Mobee job loop: offer, result, feedback/claim/accept, co-signed receipt.
...
async fn ingest_event_inner(
    ...
    if is_gift_wrap {
        ...
        if !has_p_tag {
            return Err(IngestError::Rejected(
                "invalid: gift wrap must carry at least one `p` tag".into(),
            ));
        }
    }
