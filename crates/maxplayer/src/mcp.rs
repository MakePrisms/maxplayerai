...
fn tools() -> Value {
    json!([
        ...
        {
            "name": "get_job",
            "description": format!("Read job state from the relay (offer + claims + results). Surfaces claim created_at and flags the most-recent LIVE claim. Optional include_display_names=true adds best-effort cosmetic kind-0 names; the default skips that extra network fetch and hex pubkeys remain authoritative. Optional wait_for=claim|result long-poll; timeout_secs bounds the wait, capped internally at {cap}s (values above {cap} are refused, not silently shortened) — omit timeout_secs to use the {cap}s default. Local accept-bind attached if present. Results may carry seller-claimed exec-metadata attribution (harness, model): harness is the RESOLVED id (e.g. claude-agent-acp) — a DIFFERENT vocabulary from post_job's harness labels (claude), so never string-compare the two — and every such value is an attribution, not a verification. Never invents claims/results. Set awarded_delivery_pending=true when an award is committed, the awarded seller has delivered, and settlement (accept) has not yet run — a distinct fact from live_claim_id and from accepted.", cap = job_lifecycle::WAIT_FOR_CAP_SECS),
            "inputSchema": {
                "type": "object",
                "properties": {
