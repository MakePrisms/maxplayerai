    ...
    fn under_rate_error_draft_content_carries_rate_gate_reason() {
        ...
        assert!(
            draft.tags.iter().any(|t| {
                t.0.first().map(String::as_str) == Some("reason_code")
                    && t.0.get(1).map(String::as_str) == Some("below_rate")
            }),
            "must tag below-rate refusals for buyers: {:?}",
            draft.tags
        );
    }
    ...
    #[test]
    fn offer_skip_codes_use_closed_negative_reason_vocabulary() {
        let cases = [
            (
                OfferSkip::NotAnOffer { kind: 1 },
                gateway::ReasonCode::Unparseable,
            ),
            (
                OfferSkip::ParseError {
                    reason_code: gateway::ReasonCode::UnsupportedVersion,
                    reason: "unsupported protocol version 99".into(),
                },
                gateway::ReasonCode::UnsupportedVersion,
            ),
            (
                OfferSkip::DeadlineExpired {
                    deadline_unix: 10,
                    now: 11,
                },
                gateway::ReasonCode::Unparseable,
            ...
            (
                OfferSkip::RateGate {
                    reason: "offer amount 3 sat below seller rate_sats 5".into(),
                },
                gateway::ReasonCode::BelowRate,
            ),
            (
                OfferSkip::RateGate {
                    reason: "untargeted offer refused".into(),
                },
                gateway::ReasonCode::Unparseable,
            ),
            (OfferSkip::AlreadyProcessed, gateway::ReasonCode::Unparseable),
            (
                OfferSkip::ProcessingBusy {
                    job_id: "active".into(),
                },
                gateway::ReasonCode::AtCapacity,
            ),
            (
                OfferSkip::AwaitingPaymentFull { capacity: 4 },
                gateway::ReasonCode::AtCapacity,
            ),
            (
                OfferSkip::AwaitingAwardFull { capacity: 16 },
                gateway::ReasonCode::AtCapacity,
            ),
            (
            ...
            (
                OfferSkip::ContributionMalformed {
                    reason: "bad contribution tags".into(),
                },
                gateway::ReasonCode::Unparseable,
            ),
        ];
        for (skip, expected) in cases {
            assert_eq!(skip.reason_code(), expected, "skip={skip:?}");
            assert_eq!(skip.code(), expected.as_str(), "skip={skip:?}");
        }
    }
    ...
    fn active_job_abort_error_drafts_carry_machine_readable_content() {
        ...
        for (code, content) in cases {
            let draft = error_draft(
                ...
                "buyer-pk",
                "seller-pk",
                gateway::ReasonCode::ExecutionFailed,
                content,
            );
    ...
    fn reconcile_release_error_draft_carries_machine_readable_content() {
        ...
        let draft = error_draft(
            ...
            "buyer-pk",
            "seller-pk",
            gateway::ReasonCode::DeliveryFailed,
            content,
        );
    ...
    async fn refused_offer_appends_one_refused_episode_and_leaves_journal_untouched() {
        ...
        assert_eq!(entries[0].refusal_reason_code.as_deref(), Some("unparseable"));
        assert!(
