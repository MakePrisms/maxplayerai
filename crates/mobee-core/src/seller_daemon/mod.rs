...
use crate::gateway::{
    ...
    ParsedAward, ParsedOffer, ReasonCode, TagSpec, JOB_AWARD_KIND, JOB_OFFER_KIND,
};
...
impl ErrorReasonCode {
    ...
    fn protocol_code(self) -> ReasonCode {
        match self {
            Self::AgentSpawnFailed | Self::AgentRunFailed | Self::AgentTimeout => {
                ReasonCode::ExecutionFailed
            }
            Self::GitForkFailed
            | Self::GitPushFailed
            | Self::ContributionUnsupported
            | Self::ContributionMalformed
            | Self::ClaimReleased
            | Self::Internal => ReasonCode::DeliveryFailed,
        }
    }
}
...
pub enum OfferSkip {
    ...
    NotAnOffer { kind: u16 },
    /// Offer tags did not parse or named an unsupported protocol version.
    ParseError {
...
impl OfferSkip {
    pub fn reason_code(&self) -> ReasonCode {
        match self {
            Self::NotAnOffer { .. } => ReasonCode::Unparseable,
            Self::ParseError { reason_code, .. } => *reason_code,
            Self::DeadlineExpired { .. } => ReasonCode::Unparseable,
            Self::RateGate { reason } if is_below_rate_reason(reason) => ReasonCode::BelowRate,
            Self::RateGate { .. } => ReasonCode::Unparseable,
            Self::AlreadyProcessed => ReasonCode::Unparseable,
            Self::ProcessingBusy { .. }
            | Self::AwaitingPaymentFull { .. }
            | Self::AwaitingAwardFull { .. } => ReasonCode::AtCapacity,
            Self::ContributionUnsupported => ReasonCode::UnsupportedVersion,
            Self::ContributionMalformed { .. } => ReasonCode::Unparseable,
        }
    }

    /// Machine-mappable reason code (`refusal_reason_code`). Stable protocol vocabulary.
...
impl SellerDaemon {
    ...
    async fn release_claim(&mut self, job: &ActiveJob, reason: &str) {
        ...
        self.announce(crate::announce::AnnounceEvent::released(
            ...
            &self.seller_pubkey,
            &job.job_id,
            ReasonCode::DeliveryFailed.as_str(),
            reason,
        ));
        ...
        let draft = error_draft(
            ...
            &job.buyer_pubkey,
            &self.seller_pubkey,
            ErrorReasonCode::ClaimReleased.protocol_code(),
            content,
        );
        let _ = publish_draft(&self.home, &self.keys, &draft).await;
    }
        ...
        let offer = match parse_offer(&draft) {
            ...
            Err(error) => {
                let reason_code = match error {
                    gateway::OfferParseError::UnsupportedVersion(_) => ReasonCode::UnsupportedVersion,
                    _ => ReasonCode::Unparseable,
                };
                return Ok(OfferDisposition::Skip(OfferSkip::ParseError {
                    reason_code,
    ...
    async fn fail_active(
        ...
        if let Some(active) = self.active.take() {
            ...
            let reason_code = code.protocol_code();
            ...
            self.record_errored_episode(&active, reason_code, &reason);
    ...
    fn record_errored_episode(&self, active: &ActiveJob, reason_code: ReasonCode, reason: &str) {
        let rate = require_seller_config(&self.home)
            .map(|cfg| cfg.rate_sats)
            .unwrap_or(0);
        let mut episode = Episode::new(
        ...
        episode.error_reason_code = Some(reason_code.as_str().to_owned());
...
fn is_below_rate_reason(reason: &str) -> bool {
    reason.contains("below seller rate_sats")
}
