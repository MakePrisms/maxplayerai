...
/// Buyer-authored claim award — the SELECTION of one claim, published before the seller executes.
/// A buyer-authored selection must not ride the seller's feedback kind.
pub const JOB_AWARD_KIND: u16 = 3405;
/// Buyer-authored ACCEPT — the pay-bind against one verified result, published after delivery.
///
/// Its own kind rather than a second [`JOB_AWARD_KIND`]: selection and pay-authorisation are
/// different statements, and sharing one kind made them indistinguishable on the wire, so a reader
/// could only tell them apart by counting events for a job. Two events of one kind is not a
