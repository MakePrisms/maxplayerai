...
use eframe::egui;
use mobee_core::episode::{Episode, EpisodeLog};
...
impl HomeState {
    fn resolve() -> Self {
        match mobee_core::home::default_home_dir() {
            Ok(path) => Self { path, error: None },
            Err(error) => Self {
                path: fallback_home_dir(),
                error: Some(format!(
                    "mobee_core default home resolution failed: {error}; using fallback"
                )),
            },
        }
    }
...
fn doctor_command(home: &Path) -> Command {
    ...
    // (mobee_core::home::default_home_dir). Pin it to the desktop's resolved
...
mod tests {
    ...
    use mobee_core::episode::{EpisodeKind, EpisodeOutcome};
