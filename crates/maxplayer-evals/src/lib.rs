...
mod tests {
    ...
    use maxplayer_core::event::{Event, JobExecutionStatus, JobId};
    use serde_json::json;
    ...
    fn temp_dir(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!("maxplayer-evals-{name}-{}-{id}", std::process::id()))
    }
}
