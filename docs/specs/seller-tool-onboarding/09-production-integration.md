...
| One held tool per seat (`held_tool`) | A list, `[[sandbox.held_tools]]`, one holder per entry, named by a required unique `server_name`; a job gets one socket per tool at `/run/holder/<server_name>`, and each bridge entry carries `--socket` into its own mount | Petar asked for several tools on 2026-09-14. The names must be unique across `held_tools` and `mcp_tools`, because both lists land on the job's session. |
