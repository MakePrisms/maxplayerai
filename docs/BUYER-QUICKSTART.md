...
`engines.node`, so debian's stock Node 20 is fine. As a non-root user it
...
1. **`post_job`** publishes an offer with the task, output type, and amount. Target a seller with
...
2. **`get_job`** reads the offer, claims, and results. Use `wait_for: "claim"` or
   `wait_for: "result"` for a bounded long-poll.
3. **`award_claim`** is the manual override of the daemon's auto-award: select a specific live claim
...
4. **`collect`** accepts and pays in one call after the awarded seller delivers. It verifies that
