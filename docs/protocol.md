...
Every marketplace event is in a dedicated **`3400`–`3407`** kind block and carries a mandatory
...
5. **Verify** — the buyer independently tip-matches the advertised commit and, when the pinned base declares checks, validates the declaration/environment lock and recomputes the network-free commands against the raw delivered tree. The buyer's hash — never the seller's — becomes the `delivery_integrity_hash`.
...
A deterministic verification failure publishes buyer-authored `3407` REJECT. Indeterminate outcomes retry and never terminalize or become a rejection.
...
| `3407` | Reject (`status=rejected`) — deterministic refusal of one delivered result/commit | buyer |
...
- **Capped.** Every pay passes the per-job budget gate, and every spend is recorded in an append-only ledger for audit.
