...
- **The LAN and the seller's own host services are denied, and a docker seat that has not installed the rules FAILS its boot gate** — for every docker seat, with no targeted-vs-open-pool distinction, because a job is a stranger's code whether or not the seat chose its counterparty (petar, 2026-08-18). The rules themselves are installed once, by an operator, with `sudo maxplayer sandbox-net apply`; the gate is what makes running without them impossible rather than merely inadvisable ([#797](https://github.com/MakePrisms/maxplayerai/issues/797); petar, 2026-08-14: *"this is worth locking down, to enable only internet access, not the lan as well"*). This is a **different hole from exfiltration**, and the credential design does not touch it: the container sits on a bridge network, so a stranger's job can reach whatever the seller runs locally — a Lightning node's REST port, a database, an admin UI bound to `0.0.0.0` — and anything else on the seller's LAN. A local LN node with an open REST port is a worse target than a durable credential, and nothing above is aimed at it. Denied: RFC1918 (`10/8`, `172.16/12`, `192.168/16`), CGNAT `100.64/10` (RFC 6598 — Tailscale and Headscale tailnets are private LANs by this section's own intent, and some providers serve instance metadata there rather than at `169.254.169.254`), link-local `169.254/16` including the cloud metadata endpoint, benchmarking `198.18/15`, multicast `224/4`, reserved `240/4`, and the host itself. IPv4 only, and stated rather than assumed: these are `iptables` rules, so `sandbox-net verify` refuses to call a network with IPv6 enabled contained instead of leaving a second address family unfiltered. `verify` also refuses a network that permits inter-container traffic, which is the only thing that makes job-to-job reach a decision rather than an accident of subnet choice. Public internet egress is unchanged. The single exception is one narrow pinhole to the #647 credential proxy's host port; metadata gets no pinhole at all. Connection and DNS logging ship with the rules and stay on by default — they are how anyone notices a job probing the LAN.
  ...
  **Operator note, defense in depth:** bind host services to `127.0.0.1` rather than `0.0.0.0`. A service that never listens on the bridge address is unreachable from a container even when the rules are missing or misconfigured — and the rules *can* go missing, since a host reboot, a docker daemon restart that recreates the bridge, or a `nixos-rebuild` on a declaratively-managed box all drop them silently. `maxplayer sandbox-net verify` is the check for that, and its failure is loud because the failure it detects is not.
...
- Host and LAN denial ([#797](https://github.com/MakePrisms/maxplayerai/issues/797)) — enforced by a fail-closed boot gate for every docker seat, and the only item here aimed at the seller's *own* local services rather than at what leaves the box.

Network rules are enforced **host-side**: a dedicated docker network (`[sandbox] network`) gives the rules one interface to scope to, and `maxplayer sandbox-net apply` installs them. This holds for the default host/LAN denial and for the optional domain allowlist alike. **Do not grant `NET_ADMIN` inside the sandbox for a firewall-init step** — that hands the strongest network capability back to the code the stack just stripped, and it is the naive implementation, so it is named here to be refused in review.
...
**Who the floor governs.** `runsc` seats are unaffected, for the reason above: gVisor never applies the OCI profile. So the floor binds exactly the seats with no second syscall filter behind them — Macs, and any Linux `runc` fallback. `maxplayer doctor` reads `docker version --format '{{.Server.Version}}'` (the **server**: against a remote `DOCKER_HOST` the client version describes a different machine) and reports a below-floor Engine as a **WARN for a targeted-only seat, a FAIL for an open-pool one** — the exposure split the home-permissions and containment checks already use, since serving the open pool means running code posted by strangers. A version it cannot read is reported as an explicit unknown, never passed over: a daemon that will not answer is precisely the case where a silent pass would read as "my Engine is fine".
...
- **Egress policy:** `crates/maxplayer-core/src/sandbox_net.rs` — the denied destinations, the proxy pinhole, and the `iptables` rules for both chains, rendered from one policy. Deliberately ungated, unlike the `wallet`-only modules it serves, so a default-features test run still exercises the part that decides what a stranger's job can reach. Operator surface: `crates/maxplayer/src/sandbox_net_cli.rs` (`maxplayer sandbox-net plan|apply|verify|revert`).
