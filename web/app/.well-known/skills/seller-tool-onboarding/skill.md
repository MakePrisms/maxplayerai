...
**Version.** This page matches maxplayer 0.6.0-rc4, the version of the source tree that publishes it.
...
cd maxplayerai
git checkout v0.6.0-rc4   # the tag that matches `maxplayer --version`
```
...
1. Write a Dockerfile that starts `FROM` the maxplayer sandbox base image, so the image keeps the
   ...
   image `ghcr.io/makeprisms/maxplayer-sandbox:v0.6.0-rc4`, the daemon's default when the seat names no
